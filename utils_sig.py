...
def hr_from_PSD(sig, fs, harmonics_removal=False, zero_pad = 10):
    L = len(sig)
    sig = np.pad(sig, (int(zero_pad/2*L), int(zero_pad/2*L)), 'constant')
    psd = CalculateNormPSD(fs,36,240)(torch.Tensor(sig)).numpy()
    freqs = np.linspace(36.0,240.0,len(psd))
    sorted_indices = np.argsort(-psd)

    hr1 = freqs[sorted_indices[0]]
    hr2 = freqs[sorted_indices[1]]
...
def hr_fft(sig, fs, harmonics_removal=False, zero_pad = 10):
...
def predict_heart_rate(signal, Fs, remove_harmonics = False, zero_padding = 100,min_hr=36., max_hr=240., method='fast_ideal'):

    if method == 'ideal':
        """ Zero-pad in time domain for ideal interp in freq domain
        """
        signal = signal - np.mean(signal)
        freqs, ps = compute_power_spectrum(signal, Fs, zero_pad=zero_padding)
...
# import warnings
# # Suppress warnings as exceptions
# warnings.filterwarnings("error")

def get_best_hr_900(clip, fs=30, d=0,f=0):
    import heartpy as hp
    wd, m = hp.process(clip, sample_rate = 30.0)
    return m["bpm"]

def get_best_hr_2000(clip, fs=30, d=0,f=0):
    import heartpy_2000 as hp_2000
    # hp.options["max_iterations"] = 50  # Set the desired number of iterations
    wd, m = hp_2000.process(clip, sample_rate = 30.0)
    return m["bpm"]

def get_best_hr_4000(clip, fs=30, d=0,f=0):
    import heartpy_4000 as hp_4000
    # hp.options["max_iterations"] = 50  # Set the desired number of iterations
    wd, m = hp_4000.process(clip, sample_rate = 30.0)
    return m["bpm"]

import numpy as np
from scipy.signal import find_peaks

# def calculate_IBIs(signal, sampling_rate, d= 0, f= 0):
#     # Find peaks in the RPPG signal
#     peaks, _ = find_peaks(signal)  # Adjust distance as needed
...
def get_best_hr(clip,fs = 30, d= 0, f = 0):
    import warnings
    try:
        with warnings.catch_warnings(record=True) as caught_warnings:
            hr = get_best_hr_900(clip,fs)
            # return hr
            if caught_warnings:
                with warnings.catch_warnings(record=True) as caught_warnings1:
                    hr = get_best_hr_2000(clip,fs)
                    if caught_warnings1:
                        with warnings.catch_warnings(record=True) as caught_warnings2:
                            hr = get_best_hr_4000(clip,fs)
                            if caught_warnings2:
                                print("ffff !!")
            if np.isnan(hr):
                print("corrected nan with hr fft fn")
                return hr_fft(clip,fs)
            else:
                return hr
    except:
        print("bad fit!!")
